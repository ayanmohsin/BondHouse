...
namespace ExchangeCompanySoftware
{
    public partial class frmBulkinTransit : BaseForm,IToolBar
    {
        ...
        public bool SAVE()
        {
            ...
            if (strButtonState == "ADD" || strButtonState == "EDIT")
            {
                if (ValidatingControls() == true)
                {
                    ...
                    ds = cls.SaveRecord(strButtonState, dtbDetail, strTableName, PnlMain, strTransType, strCondition, "Posted=False;Process=Green;AccountNo=" + strValue + ";BranchCode=" + General.strBranchCode + ";UserId=" + General.strUserId + "");
        ...
        public bool QUERY()
        {
            ...
            string strQuery;
            strQuery = "Select * from EX_TransBulkSalePurinTransitMaster Where BranchCode = '" + General.strBranchCode + "' AND Posted = 'false';Select a.RecNo,SNO,a.ItemCode,ItemName,ShortName,Quantity,a.Rate,Amount,EquRate,EquAmount,a.TransNo,a.BranchCode from EX_TransBulkSalePurinTransitDetail a Inner Join EX_TransBulkSalePurinTransitMaster c on a.TransNo = c.TransNo and a.RecNo = c.RecNo and a.BranchCode = c.BranchCode Inner Join EX_SetupItems b on a.ItemCode = b.ItemCode";
        ...
        public bool PRINT()
        {
            ...
            strQuery = strQuery + " Inner Join EX_SetupItems e on b.ItemCode = e.ItemCode";
            ...
            if (General.strButtonState == "ADD" || General.strButtonState == "SAVE" || General.strButtonState == "EDIT")
            {
                ...
                if (e.KeyValue == 113)
                {
                    ...
                    if (dtbDetail.Columns[dtbDetail.CurrentCell.ColumnIndex].Name == "Symbol")
                    {
                        ...
                        frmListSearch childForm = new frmListSearch(dtb, "S");
                ...
                if (dtbDetail.Columns["Quantity"].Index == e.ColumnIndex)
                {
                    ...
                    if (cls.IsNumeric(dtbDetail.Rows[e.RowIndex].Cells["Quantity"].Value.ToString()) == true && cls.IsNumeric(dtbDetail.Rows[e.RowIndex].Cells["Rate"].Value.ToString()) == true)
                    {
                       ...
                       // dtbDetail.Rows[e.RowIndex].Cells["EquAmount"].Value = (Convert.ToDouble(dtbDetail.Rows[e.RowIndex].Cells["Quantity"].Value) * Convert.ToDouble(dinumConversionRate.Value));
                        dtbDetail.Rows[e.RowIndex].Cells["EquRate"].Value = Convert.ToDouble(Convert.ToDouble(dtbDetail.Rows[e.RowIndex].Cells["Quantity"].Value)) /  Convert.ToDouble((dinumConversionRate.Value));
                    }
                    CalculateAmount();
                }
        ...
        {
            cls = new General();
            strTableName[1] = "EX_TransBulkSalePurinTransitDetail";
            strTableName[0] = "EX_TransBulkSalePurinTransitMaster";
