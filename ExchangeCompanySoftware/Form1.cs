...
namespace ExchangeCompanySoftware
{
    ...
    public partial class frmTradingVoucher : BaseForm,IToolBar 
    {
       ...
       new AutoCompleteStringCollection();
        ...
        private void PopulateCombo()
        {
            ...
            string strQuery = "Select * from EX_System Where Flag = 'T';Select * from EX_System Where Flag = 'C' and Code not in ('303','31');Select * from EX_SetupAccount Where BranchCode = '" + General.strBranchCode + "' and Status = 'A' and isTransactional = 'True' and Locked = 'False' order by Title;Select * from EX_SetupItems Where Locked = 'False' and Status = 'A';Select * from EX_System Where Flag = 'E'";
            ...
            acItemName = new AutoCompleteStringCollection();

            foreach (DataRow row in dsPopulateCombo.Tables[(int)DataPop.Item].Rows)
            {
                acItemName.Add(row["ShortName"].ToString());
             ...
             MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                       ...
                       if (Convert.ToString(dr) == "Yes")
                       {
                           ...
                           if (strButtonState == "ADD")
                           {
                               ...
                               dotxtTokenNo.Text =cls.GetDataSet(strQuery).Tables[0].Rows[0]["Token"].ToString();
                           }
