...
{
    public partial class frmContra : BaseForm,IToolBar
    {
        #region Declaration
        ...
        private void populateMaster()
        {
            ...
            string strQuery;
            strQuery = "Select * from EX_TransactionContra " + General.strStatusCondition + " and  BranchCode = '" + General.strBranchCode + "' AND Posted = 'false' and TransDate = '"+ General.dtSystemDate.ToString("dd/MMM/yyyy") +"' order by VoucherNo";
            ...
            DataSet ds = new DataSet();
             ...
             if (Convert.ToString(dr) == "Yes")
             {
                 if (strButtonState == "ADD")
                 {
                     ...
                     ditxtVoucherNo.Text = "CON-" + ditxtVoucherNo.Text + "-" + General.strBranchCode;
        ...
        public bool QUERY()
        {
            string strQuery = "Select * from EX_TransactionContra a " + General.strStatusCondition + " ";
        ...
        public bool DELETE()
        {
            strCondition = "Where VoucherNo = '" + ditxtVoucherNo.Text + "' and BranchCode = '" + General.strBranchCode + "'";
            objGetData = new ExchangeCompanySoftware.GetData.ServiceSoapClient();
            objGetData.DeleteRecord(General.strTableName,strCondition);
        ...
        private void PopulateCombo()
        {
            ...
            cls.PopulateCombo(dicboTransactionType, dsPopulateCombo.Tables[(int)DataPop.TransType], "Title", "AccountNo");
       }

        private void frmTransPaymentRec_Load(object sender, EventArgs e)
        {
            General.strTableName[0] = "EX_TransactionContra";
            General.strPKColumn = "VoucherNo";
            General.strAuthorizeTableName = General.strTableName[0];
            string strQuery = "Select AccountNo,Title from EX_SetupAccount Where isTransactional = 'True' and Locked = 'False' and BranchCode = '" + General.strBranchCode + "' and Status = 'A' and CurrencyCode = '304';Select AccountNo,Title from EX_SetupAccount Where isTransactional = 'True' and Locked = 'False' and BranchCode = '" + General.strBranchCode + "' and Status = 'A' and CurrencyCode = '304'";
        ...
        private void docboNarration_Validating(object sender, CancelEventArgs e)
        {
        }

        private void dicboTransactionType_Validated(object sender, EventArgs e)
        {
            if (dicboTransactionType.SelectedValue != null)
            {
                ...
                strQuery = strQuery + " and TransType not in (Select TransType from EX_Closing Where '" + dtDate.Value.ToString("dd/MMM/yyyy") + "' <= ClosedDate) and a.AccountNo = '" + dicboTransactionType.SelectedValue.ToString() + "'";
                strQuery = strQuery + " Group by Title,a.AccountNo,c.CurrencyCode)qry ";


                DataSet ds = new DataSet();
                ds = objGetData.GetDataSet(strQuery);
                DataRow[] dRow = ds.Tables[0].Select("AccountNo = '" + dicboTransactionType.SelectedValue.ToString() + "'");
        ...
        private void dicboAccount_Validated(object sender, EventArgs e)
        {
               if (dicboAccount.SelectedValue != null)
            {
                ...
                if (dRow.Count() > 0)
                {
                    ...
                    if (decBalance < 0)
                    {
                        donumBalanceCr.Value = -decBalance;
                        donumBalanceCr.ForeColor = Color.Blue;
                    }
                    else
                    {
                        donumBalanceCr.Value = decBalance;
                        donumBalanceCr.ForeColor = Color.Red;
                ...
                {
                    donumBalanceCr.Value = 0;
                    donumBalanceCr.ForeColor = Color.Black;
                }
            }
            else
            {
