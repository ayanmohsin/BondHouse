...
{
    public partial class frmTTIssue : BaseForm,IToolBar
    {
        GetData.ServiceSoapClient objGetData;
        General cls;
        DataTable dtSearchMaster;
        enum DataPop { Purpose,Country,Branch, Party ,Currency,OrgCurrency,AccountNo,HOAccounts,PaymentMode};
        ...
        private void populateCombo()
        {
            ...
            cls.PopulateCombo(dicboPaymentMode, dsPopulateCombo.Tables[7], "Title", "AccountNo");                
        }
        public frmTTIssue()
        {
            InitializeComponent();
        ...
        private void frmTTIssue_Load(object sender, EventArgs e)
        {
            ...
            General.strTableName[0] = "EX_TTIssuance";
            General.strTableName[1] = "EX_TTIssuanceDetail";
        ...
        public bool SAVE()
        {
            ...
            {
                ditxtTransNo.Text = "TT-" + cls.GetTransNo(strTransType) + "-" + General.strBranchCode;
                for (int i = 0; i < dtbDetail.Rows.Count; i++)
                {
            ...
            string strRemarls = "TT Issue for " + ditxtAppfavour.Text + " of " + dinumQuantity.Value.ToString() + " for " + dicboCurrency.Text + " of " + dinumTotalAmountRec.Value.ToString();
            dtbDetail.Rows.Clear();
            if (General.strBranchCode != General.strHeadOfficeCode)
            {
                ...
                dtbDetail.Rows[1].Cells["AccountNo"].Value = cls.strControlAccount("ExchangeGainLossTT");
                ...
                dtbDetail.Rows[2].Cells["AccountNo"].Value = cls.strControlAccount("TTCharges");
        ...
        public bool EDIT()
        {
            if (txtStatus.Text == "A")
            {
                ...
                MessageBox.Show("Authorized Record could not be Edited. Delete this Record And Insert Again", "Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
	        {
        ...
        public bool QUERY()
        {
            ...
            string strQuery;
            strQuery = "Select * from EX_TTIssuance a " + General.strStatusCondition + "  and BranchCode = '" + General.strBranchCode + "' ";
            ...
            strQuery = strQuery + ";Select c.Sno,c.TransNo,c.AccountNo,c.Debit,c.Credit,b.Title,c.BranchCode from EX_TTIssuanceDetail c Inner Join EX_TTIssuance  a on a.TransNo = c.TransNo and a.BranchCode = c.BranchCode and a.RecNo = c.RecNo  Inner Join EX_SetupAccount b on c.Accountno = b.AccountNo and a.BranchCode = b.BranchCode and b.Status = 'A' Where c.BranchCode = '" + General.strBranchCode + "'";
            if (General.strFormQueryCriteria != "")
            {
                strQuery = strQuery + General.strFormQueryCriteria;
            }
        ...
        public bool PRINT()
        {
            if (txtStatus.Text == "A")
            {
                if (ditxtTransNo.Text != "")
                {
                    ...
                    strQuery = strQuery + " Select a.TransDate,a.TransNo, ";
                    strQuery = strQuery + " a.FTTNo,AppName,AppAddress,AppPhone,AppNic,AppSWD,RecFavour, ";
                    strQuery = strQuery + " RecSWD,RecNic,RecBank,RecBranch,RecRoutingNo,RecAccountNo,RecBankCrosse, ";
                    strQuery = strQuery + " HOBank,Rate,Quantity,Amount,AmountRec,BaseRate,OrgRate,HOCharges,Charges, ";
                    strQuery = strQuery + " WHT,a.Remarks,l.Title as Accounting,Debit,Credit ";
                    strQuery = strQuery + " ,c.BranchName  as [Report Branch],d.BranchName,e.ItemName as CURRNAME,e.ShortName as CURRSNAME, ";
                    strQuery = strQuery + " f.Description,g.Purpose,h.CustName,i.Title as HOBANKNAME,j.ItemName as ORGCURRNAME,j.ShortName as ORGCURRSNAME, ";
                    strQuery = strQuery + " Round(case When a.CurrencyCode = '2' then Quantity else AmountRec/(Select Description from EX_System Where Flag = 'E') end,2) as EQUSD ";
                    strQuery = strQuery + " from EX_TTIssuance a ";
                    strQuery = strQuery + " Inner Join EX_TTIssuanceDetail b on a.TransNo = b.TransNo and a.RecNo = b.RecNo and a.BranchCode = b.BranchCode ";
                    ...
                    strQuery = strQuery + " Inner Join EX_SetupCustomer h on a.RecPartyCode = h.CustCode and h.Status = 'A' ";
                    strQuery = strQuery + " Inner Join EX_SetupAccount i on a.HOBank = i.AccountNo and i.Status = 'A' ";
                    strQuery = strQuery + " Inner Join EX_SetupAccount l on b.AccountNo = l.AccountNo and l.Status = 'A' ";
                    ...
                    dtb = objGetData.GetDataSet(strQuery).Tables[0];
                    rptTTTicket devrep = new rptTTTicket();
                    dtb = objGetData.GetDataSet(strQuery).Tables[0];
        ...
        private void frmTTIssue_Activated(object sender, EventArgs e)
        {
