    ...
    public partial class frmCurrencyPosition : BaseForm, IToolBar
    {
        ...
        private void PopulateCurrencyPosition()
        {
            ...
            try
            {
                ...
                strQuery2 = strQuery2 + " and a.BranchCode in (" + strBranch + ") and a.CurrencyCode != '304' and a.Transdate <= '" + dtSystemDate.Value + "' ";
                strQuery2 = strQuery2 + " Group by ";
                ...
                strQuery = strQuery + " Where AccountNo in ((Select Cashinhand From EX_ControlAccounts Where BranchCode = a.BranchCode)) and BranchCode in (" + strBranch + ") and Status = 'A' and a.TransDate <= '" + dtSystemDate.Value.ToString("dd/MMM/yyyy") + "'";
                string strQuery1 = "Select Round(Sum(case When Flag = 'D' then Debit else - Credit end),2) as CashinHand from EX_PrsTransactions a";
                strQuery1 = strQuery1 + " Where AccountNo in ((Select Cashinvault From EX_ControlAccounts Where BranchCode = a.BranchCode)) and BranchCode in (" + strBranch + ") and Status = 'A' and BranchCode in (" + strBranch + ") and a.TransDate <= '" + dtSystemDate.Value.ToString("dd/MMM/yyyy") + "' ";
